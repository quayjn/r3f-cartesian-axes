      ...
      for (var i = tickspacing; i <= depth; i += tickspacing) {
        ...
        if (includeLabels) {
          strLabelPos = (
            <Text
              key={`labelx-${i}`}
              position={[0, -(1 + ticklength / 2), 0]}
              rotation={[0, 0, 0]}
              anchorX="center"
              anchorY="top"
              color={xColor}
              fontSize={(thickness * tickspacing) / 3}
            >
              {i.toFixed(0)}
            </Text>
          );
        ...
        ticksX.push(
          ...
          >
            <cylinderGeometry
              args={[thickness / 2, thickness / 2, ticklength, 16]}
            />
            <meshBasicMaterial
              color={xColor}
              depthWrite={true}
  ...
  if (includeZ) {
    axes.push(
      <mesh position={[0, 0, 0]} rotation={[Math.PI / 2, 0, 0]} key={`axisz`}>
        <cylinderGeometry args={[thickness, thickness, 2 * depth, 16]} />
        <meshBasicMaterial color={zColor} transparent={true} depthTest={true} />
      </mesh>
    );
    if (tickspacing > 0) {
      for (var i = tickspacing; i <= depth; i += tickspacing) {
